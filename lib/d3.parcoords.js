d3.parcoords = function(config) {
  var __ = {
    ...
    nullValueSeparator: "bottom", //"undefined", // set to "top" or "bottom"
    nullValueSeparatorPadding: { top: 8, right: 0, bottom: 20, left: 0 },
    color: "#069",
...
pc.autoscale = function() {
  ...
  var defaultScales = {
    ...
    "string": function(k) {
      ...
      // domain = Object.getOwnPropertyNames(counts).sort();  //hmhm: sorting by string 
      // domain = Object.getOwnPropertyNames(counts).sort(function(a, b) {
      //   return counts[a] - counts[b];             // sorted by count of records
      // });
  ...
  ctx.foreground.lineWidth = 1.0;
  ctx.foreground.globalCompositeOperation = __.composite;
  ctx.foreground.globalAlpha = __.alpha;
  ctx.foreground.scale(devicePixelRatio, devicePixelRatio);
  ctx.brushed.strokeStyle = __.brushedColor;
  ctx.brushed.lineWidth = 1.5;
...
// 	d3.entries(__.dimensions).forEach(function(p, i) {  //p isn't really p
// 		if (i == 0) {
// 			ctx.moveTo(position(p.key), d[p.key] == '' ? getNullPosition() : __.dimensions[p.key].yscale(d[p.key]));
// 		} else {
// 			ctx.lineTo(position(p.key), d[p.key] == '' ? getNullPosition() : __.dimensions[p.key].yscale(d[p.key]));
// 		}
// 	});
// };
...
function updateLabel(dimension,idx) {
  // brushUpdated(selected0()) //for testing HMHM
  d3.select("#labelDetail").text(idx +': ' +dimension);
}

function dimensionLabels_old(d) {
  return __.dimensions[d].title ? __.dimensions[d].title : d;  // dimension display names, hmhm:old
}
function dimensionLabels(d) {
  return __.dimensions[d].title ? Label_1st(__.dimensions[d].title) : Label_1st(d);  // dimension display names hmhm:new
}
function Label_1st(d) {
  var l 
  if (d.search(':') == -1) {
    l = 10;
  } else {
  ...
  return d.substr(0,l);  // dimension display 1st names
}
function Label_2nd(d) {
  return d.substr(d.search(':')+1,100);  // dimension display 2nd names
}

var matchDim = function(d) { 
  var dimL = d3.keys(__.dimensions)
  for (let i=0; i < dimL.length; i++) {    //forEach not working to break 
    if (dimL[i].includes(d))  {return dimL[i]; break}
  }
} 

function case_(i) {                                                             //hmhm: modified based on selected
  var actives = __.select_0[i][1].map(function(p) {
    if (d3.keys(__.dimensions).includes(p[0])) {
      return p[0]                                                               // full match
    } else {
      return matchDim(p[0])                                                     // partial match
    }
  })
      extents = __.select_0[i][1].map(function(p) {return p.slice(1,p.lenght)});
...
// 	if (actives.length === 0) return __.data;
      ...
      "string": function(d,p,dimension) {
        // return extents[dimension][0] <= __.dimensions[p].yscale(d[p]) && __.dimensions[p].yscale(d[p]) <= extents[dimension][1]
        return extents[dimension].includes(d[p]) //contain | include
      }
    };
  var within0 = {   // simplified for testing
    "number": function(d) {
      return 0.5 <= d['CPS2'] && d['CPS2'] <= 0.85
    } //,
  };
    ...
    .filter(function(d) {
      ...
      case "AND":
        return actives.every(function(p, dimension) {
          // return within[__.dimensions['CPS2'].type](d);
...
d3.select("#observ") //hmhm: add a dropdown list for cases
  .selectAll("div")
  .data(__.select_0)
  .enter().append("div")
  .attr("id", function(d,i) {return "obs_" + i})
  .html(function(d) { 
	   if (d.length <= 2) {
		return "";
	   } else {
...
d3.selectAll("div[id^='obs_']").attr("hidden",true)
//   var dropdown = d3.select("#_03_") //hmhm: add a dropdown list for cases
//   .append("select")
//   .attr("id", "caseselector")
//   .selectAll("option")
//   .data(__.select_0)
//   .enter().append("option")
//   .text(function(d) { return d[0]; })
//   .attr("value", function (d, i) {return i;})
//   // .on("change", updateCase)

  var dropdown = d3.select("#caseselector") //hmhm: add a dropdown list for cases
  .selectAll("option")
  .data(__.select_0)
  .enter().append("option")
  .text(function(d) { return d[0]; })
  ...
  // .on("change", updateCase)
	...
	d3.select("#caseselector")
	.on("change", function(d) {
    index = +this.value
    // updateCase(index)
    brushUpdated(case_(index));
    d3.selectAll("div[id^='obs_']").attr("hidden",true);
    d3.selectAll("#obs_"+index).attr("hidden",null);
    d3.select("#filterTxt").text(" ... filter: [" + config_hm.select_0[index][1].join(" && ") +"]");
    d3.select("#sel_").text("[" + config_hm.select_0[index][1].join(" && ") +"]");
	})
...
function brushUpdated(newSelection) {
  ...
  data_sel = newSelection;  //hmhm: to expose selected / filtered data, difined in index.html line 83
//  test = newSelection;
//  d3.keys(newSelection[0]).filter(function(k) {return (__.rangeDim[k] = Number(newSelection[0][k]) && d3.extent(newSelection,function(f){ return +f[k];}))});  // create max_min[k]
//   d3.keys(newSelection[0]).filter(function(k) {return (__.rangeDim[k] = d3.extent(newSelection,function(f){ return +f[k];}))});  //hmhm create max_min[k]
  d3.keys(newSelection[0]).forEach(function(k) {(__.rangeDim[k] = d3.extent(newSelection,function(f){ return +f[k];}))});  //hmhm create max_min[k]
  d3.keys(newSelection[0]).filter(function(k) {return (__.rangeAvg[k] = d3.mean(newSelection,function(f){ return +f[k];}))});  //hmhm create average[k]
  if (Number(d3.select("#base").text()) > 0) { 
	var lth = Number(d3.select("#base").text());
	} else {
	var lth = __.data.length;	
	}
  //hmhm: adding counts, probability, [max avg min] 
  d3.select("#counter").text((100*newSelection.length/lth).toFixed(2) + '%, [' + newSelection.length + '] out of  [' + lth + ']');
  d3.selectAll("text.lab_min").each(function(d) {
	  //if (Math.max(Math.abs(__.rangeDim[d][1]),Math.abs(__.rangeDim[d][0])) > 100) {
	  // if (Math.abs(__.rangeDim[d][0]) > 100) {
	  //  this.innerHTML = __.rangeDim[d][0].toFixed(0);
	  // } else {
	  // this.innerHTML = __.rangeDim[d][0].toFixed(3);
	  // }
	  if (__.rangeDim[d][0] != undefined) {
		this.innerHTML = __.rangeDim[d][0].toPrecision(4);
	  }
  });
  d3.selectAll("text.lab_max").each(function(d) {
      //this.innerHTML = d3.extent(test,function(f){return +f[d];})[1];
	  if (__.rangeDim[d][0] != undefined) {
		this.innerHTML = __.rangeDim[d][1].toPrecision(4);
	  }
  });
  d3.selectAll("text.lab_avg").each(function(d) {
	  if (__.rangeDim[d][0] != undefined) {
		this.innerHTML = __.rangeAvg[d].toPrecision(4);
	  }
  });
  
  var sel_txt1 ="|"
  sel_txt ="|"
  brsRect = document.getElementsByClassName('extent')
  //d3.selectAll(".extent").each(function(d) {
  //	sel_txt = sel_txt + d
  //});
  
  var i;
  for (i = 0; i < brsRect.length; i++) { 
    if (brsRect[i].height.animVal.value > 0) {
		if (__.rangeDim[brsRect[i].__data__][0] == undefined) {
		var txt = Label_1st(brsRect[i].__data__) + ":" + getColUniq(newSelection,brsRect[i].__data__) + " |";
		} else {
		var txt = Label_1st(brsRect[i].__data__) + ":" + __.rangeDim[brsRect[i].__data__][0] + "-" + __.rangeDim[brsRect[i].__data__][1] + " |";
		}
		sel_txt += txt;
		if (!base_txt.includes("|"+ brsRect[i].__data__+":")) {
			sel_txt1 += txt;
		}
	}
  }

  d3.select("#sel_").text(sel_txt1)

    d3.selectAll("div[id^='obs_']").attr("hidden",true);
    d3.select("#filterTxt").text("");
